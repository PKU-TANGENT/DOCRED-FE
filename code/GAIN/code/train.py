...
def train(opt):
    ...
    # 同时 model = GAIN_BERT(opt)/GAIN_Glove(opt)  也不相同
    ...
    for epoch in range(start_epoch, opt.epoch + 1):
        ...
        for ii, d in enumerate(train_loader):
            relation_multi_label = d['relation_multi_label'] # size = ([batch_size,95,97])
            relation_mask = d['relation_mask'] # size = (batch_size,95)
            relation_label = d['relation_label'] # size = (batch_size,95)
            ...
            if global_step % log_step == 0:
                ...
                logging(
                    '| epoch {:2d} | step {:4d} |  ms/b {:5.2f} | train loss {:5.3f} | NA acc: {:4.2f} | not NA acc: {:4.2f}  | tot acc: {:4.2f} '.format(
                        ...
                        acc_total.get()))
                # pbar.set_postfix_str('loss ={: f}'.format(cur_loss*1000))
                total_loss = 0
                start_time = time.time()
            # pbar.update(1)
        if epoch % opt.dev_period == 0 and epoch >=180:
            logging('-' * 89)
